...
def analyze(run_dir: Path) -> dict[str, object]:
    ...
    with dataref_stats_path.open("w", encoding="utf-8", newline="") as f:
        fieldnames = [
            "path", "group", "unit_hint", "exists", "type_names", "count", "blank_count",
    ...
    with derived_path.open("w", encoding="utf-8", newline="") as f:
        fieldnames = ["frame_id", *KEY_COLUMNS.values()]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
    ...
    sim_duration = sim_times[-1] - sim_times[0] if len(sim_times) >= 2 else math.nan
